/**
 * source === "sephora" ve year === 2026 olan kayıtlarda yılı metinden çıkarır;
 * bulunamazsa yanlış placeholder'ı kaldırır (year alanı silinir — UI göstermez).
 *
...
function main() {
  ...
  for (const p of list) {
    if (p.source !== "sephora" || p.year !== PLACEHOLDER) continue;
  ...
  console.log(
    DRY
      ? `[dry-run] Metinden yıl atanacak: ${fromText}, year kaldırılacak: ${removed}`
      : `Metinden yıl atandı: ${fromText}, placeholder year kaldırıldı: ${removed}`
  );
